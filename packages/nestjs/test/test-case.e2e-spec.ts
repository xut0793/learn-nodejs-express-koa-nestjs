...
describe('TestCaseModule (e2e)', () => {
  let app: INestApplication;
  let request: supertest.SuperTest<supertest.Test>;

  beforeAll(async () => {
    const moduleTestCase: TestingModule = await Test.createTestingModule({
      imports: [AppModule],
    }).compile();

    app = moduleTestCase.createNestApplication();
    await app.init();
