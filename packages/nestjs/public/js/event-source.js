...
class SSE {
   ...
   *
   * 第一种方式需要频繁的开启定时器，第二种需要维持一个长时间定时任务，但每次消息处理只需要更新一个变量值而已。就cpu 的开销时间，第一种是第二种的几百倍。但第一种的开销占cpu总开销比例也很小。所以并不会成为性能瓶颈，并且时间间隔更精确
...
const sseInterval = new SSE('/sse/interval');
const sseEmit = new SSE('/sse/subscribe');

const startIntervalBtn = document.getElementById('start-interval');
const startEmitBtn = document.getElementById('start-emit');
const stopBtn = document.getElementById('stop');
const emitBtn = document.getElementById('emit');

startIntervalBtn.addEventListener('click', () => sseInterval.connect());
startEmitBtn.addEventListener('click', () => sseEmit.connect());
stopBtn.addEventListener('click', () => {
  sseInterval.disconnect();
  sseEmit.disconnect();
});
emitBtn.addEventListener('click', () => {
  fetch('/sse/broadcast?a=1&b=2');
});
