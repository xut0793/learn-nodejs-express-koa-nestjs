...
大多数内存管理的问题都在这个彩放和回收阶段。在这里最艰难的任务是找到“哪些被分配的内存确实已经不再需要了”。不同应用程序对内存分配和内存回收的处理方式的不同，分为：
...
但同时也受到V8的限制，尤其是内存使用上。一般的后台开发语言中，内存使用的大小几乎没有限制。但是，V8 最初是为浏览器的打造的，在V8引擎的实现中，64位系统只可以操纵 1.4GB 内存，32位系统只可以操纵 0.7GB 内存。

在 Nodejs 中运行程序，内存使用限制大约是 4GB，不同的机器可能会有不同。具体使用值可以通过以下代码查看：
...
在 V8 中，将可用的内存块分为栈内存和堆内存。

- Stack memory 栈内存：存储静态数据、方法和函数帧、基元值以及指向存储对象的指针。它们以 LIFO(Last In First Out) 后近先出顺序推送和弹出，从栈中弹出就会自动释放相关的栈内存，所以这里没有什么太多担心的问题。
- Heap memory 堆内存：存储着栈中指针指向的实际对象。因为 JavaScript 中的所有内容都是一个对象，这意味着所有动态数据（如数组、函数、闭包、集合和所有类的实例等）都存储在堆中。因此堆是应用程序使用最大的内存块，也是垃圾回收 （GC: Garbage Collection） 发生的地方。
...
![Mark-Sweep](../image/mark-sweep.webp)
...
![Mark-Compact](../image/mark-compact.webp)
