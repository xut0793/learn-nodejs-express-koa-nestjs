...
1. 在 HTTP 请求`Content-Type：application/json;charset=utf-8`, 在 script 标签中设置`<script charset="utf-8" scr=""></script>`, HTML 中`<meta charset='utf-8'>`的意义
1. 字符转义格式：在 XML/HTML 中：`&#8733;` `&#x221d;`，在 JS 中使用`\u`：`\u221d`
1. 在 Javascript 中，用如何处理字符串在内存中的存储的
...
这个 0x5144 就叫做字符“兄”的 Unicode 码位，也称为代码点，简称码点。
...
在我们的认知里：在 javascript 语言的数据类型分为基础类型和引用类型，内存管理分为栈内存和堆内存。然后基础类型存储在栈内存，引用类型存储堆内存。

然后在 javascript 语言中将 string 类型归为基础类型，所以大家没有深究时，通常会认为 string 类型的数据也在栈内存空间中进行管理。但是大家要知道，V8 默认栈内存是 984 Kib ，那如果从文件中读取结果赋值为一个超长字符串 > 984Kib 能装的进 栈内存 吗？

你可以在浏览器用调试控制台中的内存快照查看字符串的存储，结论就是：字符串的内容存于堆内存中，指针存于栈内存中，且相同的字符串指向同一个堆内存地址。并且当我们新建一个字符串时，V8会从内存中查找一下是否已经有存在的一样的字符串，找到的话直接复用。如果找不到的话，则开辟一块新的内存空间来存这个字符串，并把地址赋给变量。

> v8内部有一个名为 stringTable 的 hashmap 缓存了所有字符串，在V8引擎扫描我们的代码，转换抽象语法树时，每遇到一个字符串，会根据其特征换算为一个 hash 值 ，插入到 hashmap 中。在之后如果遇到了 hash 值一致的字符串，会优先从里面取出来进行比对，一致的话就不会生成新字符串类。缓存字符串时，根据字符串不同采取不同 hash 方式。

javascript 这点上有点反直觉，像其它后端语言，如 java / rust 对比较严格区分单个字符和字符串的。

> 引用链接：[「字符串」存在「栈内存」中？](https://segmentfault.com/a/1190000041298965)
